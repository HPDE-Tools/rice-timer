 ...
 */
void ringbuf_get_sizes(unsigned nworkers, size_t* ringbuf_size, size_t* ringbuf_worker_size) {
...
ssize_t ringbuf_acquire(ringbuf_t* rbuf, ringbuf_worker_t* w, size_t len) {
  ...
  do {
    ...
    atomic_store_explicit(&w->seen_off, next | WRAP_LOCK_BIT, memory_order_relaxed);
   ...
   */
  atomic_store_explicit(&w->seen_off, w->seen_off & ~WRAP_LOCK_BIT, memory_order_relaxed);
  ...
  if (__predict_false(target & WRAP_LOCK_BIT)) {
     ...
     */
    atomic_store_explicit(&rbuf->next, (target & ~WRAP_LOCK_BIT), memory_order_release);
  }
...
retry:
  ...
  for (unsigned i = 0; i < rbuf->nworkers; i++) {
     ...
     */
    if (!atomic_load_explicit(&w->registered, memory_order_relaxed)) continue;
  ...
  if (next < written) {
    ...
    if (ready == RBUF_OFF_MAX && written == end) {
       ...
       */
      written = 0;
      atomic_store_explicit(&rbuf->written, written, memory_order_release);
      goto retry;
    }
